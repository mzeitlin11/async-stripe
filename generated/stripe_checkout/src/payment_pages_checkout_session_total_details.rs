...
pub struct PaymentPagesCheckoutSessionTotalDetailsBuilder {
    ...
    breakdown: Option<Option<stripe_checkout::PaymentPagesCheckoutSessionTotalDetailsResourceBreakdown>>,
}
...
const _: () = {
    ...
    impl Visitor for Place<PaymentPagesCheckoutSessionTotalDetails> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: PaymentPagesCheckoutSessionTotalDetailsBuilder::deser_default() }))
        }
    }
