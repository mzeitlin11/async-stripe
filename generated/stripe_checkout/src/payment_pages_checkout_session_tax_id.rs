...
const _: () = {
    ...
    impl Visitor for Place<PaymentPagesCheckoutSessionTaxId> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: PaymentPagesCheckoutSessionTaxIdBuilder::deser_default() }))
        }
    }
...
impl std::fmt::Debug for PaymentPagesCheckoutSessionTaxIdType {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.write_str(self.as_str())
    }
}
...
impl<'de> serde::Deserialize<'de> for PaymentPagesCheckoutSessionTaxIdType {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        Self::from_str(&s).map_err(|_| serde::de::Error::custom("Unknown value for PaymentPagesCheckoutSessionTaxIdType"))
    }
}
#[cfg(feature = "min-ser")]
...
impl miniserde::de::Visitor for crate::Place<PaymentPagesCheckoutSessionTaxIdType> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(PaymentPagesCheckoutSessionTaxIdType::from_str(s).map_err(|_| miniserde::Error)?);
        Ok(())
    }
}
