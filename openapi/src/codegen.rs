...
pub fn gen_prelude(state: &FileGenerator) -> String {
    let name = state.name.clone();
    let object = &name;
            ...
            match param_name {
                ...
                _ => {
                    ...
                    if let Some((use_path, rust_type)) =
                        field_to_rust_type(params_schema.as_str(), param_name)
                    {
                        print_doc(out);
