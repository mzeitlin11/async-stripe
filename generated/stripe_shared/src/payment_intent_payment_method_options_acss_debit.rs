...
pub struct PaymentIntentPaymentMethodOptionsAcssDebit {
    pub mandate_options: Option<stripe_shared::PaymentIntentPaymentMethodOptionsMandateOptionsAcssDebit>,
...
pub struct PaymentIntentPaymentMethodOptionsAcssDebitBuilder {
    mandate_options: Option<Option<stripe_shared::PaymentIntentPaymentMethodOptionsMandateOptionsAcssDebit>>,
    setup_future_usage: Option<Option<PaymentIntentPaymentMethodOptionsAcssDebitSetupFutureUsage>>,
    verification_method: Option<Option<PaymentIntentPaymentMethodOptionsAcssDebitVerificationMethod>>,
}
...
const _: () = {
    ...
    impl Visitor for Place<PaymentIntentPaymentMethodOptionsAcssDebit> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: PaymentIntentPaymentMethodOptionsAcssDebitBuilder::deser_default() }))
        }
    }
    ...
    impl MapBuilder for PaymentIntentPaymentMethodOptionsAcssDebitBuilder {
        ...
        fn deser_default() -> Self {
            Self { mandate_options: Deserialize::default(), setup_future_usage: Deserialize::default(), verification_method: Deserialize::default() }
        }
...
#[cfg(feature = "min-ser")]
impl miniserde::de::Visitor for crate::Place<PaymentIntentPaymentMethodOptionsAcssDebitSetupFutureUsage> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(PaymentIntentPaymentMethodOptionsAcssDebitSetupFutureUsage::from_str(s).map_err(|_| miniserde::Error)?);
        Ok(())
    }
}
/// Bank account verification method.
...
#[cfg(feature = "min-ser")]
impl miniserde::de::Visitor for crate::Place<PaymentIntentPaymentMethodOptionsAcssDebitVerificationMethod> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(PaymentIntentPaymentMethodOptionsAcssDebitVerificationMethod::from_str(s).map_err(|_| miniserde::Error)?);
        Ok(())
    }
}
