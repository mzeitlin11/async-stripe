...
const _: () = {
    ...
    impl Visitor for Place<TerminalReaderReaderResourceTippingConfig> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: TerminalReaderReaderResourceTippingConfigBuilder::deser_default() }))
        }
    }
    ...
    impl MapBuilder for TerminalReaderReaderResourceTippingConfigBuilder {
        ...
        fn take_out(&mut self) -> Option<Self::Out> {
            let amount_eligible = self.amount_eligible.take()?;
