...
pub struct TerminalConfigurationConfigurationResourceTippingBuilder {
    aud: Option<Option<stripe_terminal::TerminalConfigurationConfigurationResourceCurrencySpecificConfig>>,
...
const _: () = {
    ...
    impl Visitor for Place<TerminalConfigurationConfigurationResourceTipping> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: TerminalConfigurationConfigurationResourceTippingBuilder::deser_default() }))
        }
    }
    ...
    impl MapBuilder for TerminalConfigurationConfigurationResourceTippingBuilder {
        ...
        fn key(&mut self, k: &str) -> miniserde::Result<&mut dyn Visitor> {
            ...
            match k {
                ...
                "chf" => Ok(Deserialize::begin(&mut self.chf)),
                "czk" => Ok(Deserialize::begin(&mut self.czk)),
                "dkk" => Ok(Deserialize::begin(&mut self.dkk)),
                "eur" => Ok(Deserialize::begin(&mut self.eur)),
                "gbp" => Ok(Deserialize::begin(&mut self.gbp)),
                "hkd" => Ok(Deserialize::begin(&mut self.hkd)),
                "myr" => Ok(Deserialize::begin(&mut self.myr)),
                "nok" => Ok(Deserialize::begin(&mut self.nok)),
                "nzd" => Ok(Deserialize::begin(&mut self.nzd)),
                "sek" => Ok(Deserialize::begin(&mut self.sek)),
                "sgd" => Ok(Deserialize::begin(&mut self.sgd)),
                "usd" => Ok(Deserialize::begin(&mut self.usd)),
        ...
        fn take_out(&mut self) -> Option<Self::Out> {
            let aud = self.aud.take()?;
            let cad = self.cad.take()?;
            let chf = self.chf.take()?;
            let czk = self.czk.take()?;
            let dkk = self.dkk.take()?;
            let eur = self.eur.take()?;
            let gbp = self.gbp.take()?;
            let hkd = self.hkd.take()?;
            let myr = self.myr.take()?;
            let nok = self.nok.take()?;
            let nzd = self.nzd.take()?;
            let sek = self.sek.take()?;
            let sgd = self.sgd.take()?;
            let usd = self.usd.take()?;
