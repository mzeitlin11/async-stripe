...
const _: () = {
    ...
    impl Visitor for Place<PaymentMethodDetailsAcssDebit> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: PaymentMethodDetailsAcssDebitBuilder::deser_default() }))
        }
    }
    ...
    impl MapBuilder for PaymentMethodDetailsAcssDebitBuilder {
        ...
        fn take_out(&mut self) -> Option<Self::Out> {
            ...
            Some(Self::Out { bank_name, fingerprint, institution_number, last4, mandate, transit_number })
        }
    }
