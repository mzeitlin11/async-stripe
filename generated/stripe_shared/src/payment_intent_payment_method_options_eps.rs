...
#[cfg(feature = "min-ser")]
pub struct PaymentIntentPaymentMethodOptionsEpsBuilder {
    setup_future_usage: Option<Option<PaymentIntentPaymentMethodOptionsEpsSetupFutureUsage>>,
}
...
const _: () = {
    ...
    impl Deserialize for PaymentIntentPaymentMethodOptionsEps {
    ...
    struct Builder<'a> {
        out: &'a mut Option<PaymentIntentPaymentMethodOptionsEps>,
        builder: PaymentIntentPaymentMethodOptionsEpsBuilder,
    }

    impl Visitor for Place<PaymentIntentPaymentMethodOptionsEps> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: PaymentIntentPaymentMethodOptionsEpsBuilder::deser_default() }))
        }
    }

    impl MapBuilder for PaymentIntentPaymentMethodOptionsEpsBuilder {
        type Out = PaymentIntentPaymentMethodOptionsEps;
    ...
    impl ObjectDeser for PaymentIntentPaymentMethodOptionsEps {
...
impl<'de> serde::Deserialize<'de> for PaymentIntentPaymentMethodOptionsEpsSetupFutureUsage {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        Self::from_str(&s).map_err(|_| serde::de::Error::custom("Unknown value for PaymentIntentPaymentMethodOptionsEpsSetupFutureUsage"))
    }
}
#[cfg(feature = "min-ser")]
impl miniserde::Deserialize for PaymentIntentPaymentMethodOptionsEpsSetupFutureUsage {
...
#[cfg(feature = "min-ser")]
impl miniserde::de::Visitor for crate::Place<PaymentIntentPaymentMethodOptionsEpsSetupFutureUsage> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(PaymentIntentPaymentMethodOptionsEpsSetupFutureUsage::from_str(s).map_err(|_| miniserde::Error)?);
        Ok(())
    }
}
