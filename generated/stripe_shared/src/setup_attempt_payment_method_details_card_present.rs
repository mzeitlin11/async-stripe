...
const _: () = {
    ...
    impl Visitor for Place<SetupAttemptPaymentMethodDetailsCardPresent> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: SetupAttemptPaymentMethodDetailsCardPresentBuilder::deser_default() }))
        }
    }
    ...
    impl MapBuilder for SetupAttemptPaymentMethodDetailsCardPresentBuilder {
        ...
        fn deser_default() -> Self {
            Self { generated_card: Deserialize::default() }
        }

        fn take_out(&mut self) -> Option<Self::Out> {
            let generated_card = self.generated_card.take()?;

            Some(Self::Out { generated_card })
        }
    }
