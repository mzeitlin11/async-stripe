...
const _: () = {
    ...
    impl Visitor for Place<TerminalReaderReaderResourceSetReaderDisplayAction> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: TerminalReaderReaderResourceSetReaderDisplayActionBuilder::deser_default() }))
        }
    }
    ...
    impl MapBuilder for TerminalReaderReaderResourceSetReaderDisplayActionBuilder {
        ...
        fn key(&mut self, k: &str) -> miniserde::Result<&mut dyn Visitor> {
            match k {
                "cart" => Ok(Deserialize::begin(&mut self.cart)),
...
impl<'de> serde::Deserialize<'de> for TerminalReaderReaderResourceSetReaderDisplayActionType {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        Self::from_str(&s).map_err(|_| serde::de::Error::custom("Unknown value for TerminalReaderReaderResourceSetReaderDisplayActionType"))
    }
}
#[cfg(feature = "min-ser")]
...
#[cfg(feature = "min-ser")]
impl miniserde::de::Visitor for crate::Place<TerminalReaderReaderResourceSetReaderDisplayActionType> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(TerminalReaderReaderResourceSetReaderDisplayActionType::from_str(s).map_err(|_| miniserde::Error)?);
        Ok(())
    }
}
