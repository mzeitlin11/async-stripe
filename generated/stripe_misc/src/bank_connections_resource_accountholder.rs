...
const _: () = {
    ...
    impl Visitor for Place<BankConnectionsResourceAccountholder> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: BankConnectionsResourceAccountholderBuilder::deser_default() }))
        }
    }
    ...
    impl MapBuilder for BankConnectionsResourceAccountholderBuilder {
        ...
        fn deser_default() -> Self {
            Self { account: Deserialize::default(), customer: Deserialize::default(), type_: Deserialize::default() }
        }
...
impl<'de> serde::Deserialize<'de> for BankConnectionsResourceAccountholderType {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        Self::from_str(&s).map_err(|_| serde::de::Error::custom("Unknown value for BankConnectionsResourceAccountholderType"))
    }
}
#[cfg(feature = "min-ser")]
...
impl miniserde::de::Visitor for crate::Place<BankConnectionsResourceAccountholderType> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(BankConnectionsResourceAccountholderType::from_str(s).map_err(|_| miniserde::Error)?);
        Ok(())
    }
}
