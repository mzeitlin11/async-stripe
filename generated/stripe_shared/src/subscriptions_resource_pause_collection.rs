...
const _: () = {
    ...
    impl Visitor for Place<SubscriptionsResourcePauseCollection> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: SubscriptionsResourcePauseCollectionBuilder::deser_default() }))
        }
    }
    ...
    impl MapBuilder for SubscriptionsResourcePauseCollectionBuilder {
        ...
        fn key(&mut self, k: &str) -> miniserde::Result<&mut dyn Visitor> {
            ...
            match k {
                "behavior" => Ok(Deserialize::begin(&mut self.behavior)),
                "resumes_at" => Ok(Deserialize::begin(&mut self.resumes_at)),
        ...
        fn take_out(&mut self) -> Option<Self::Out> {
            ...
            let resumes_at = self.resumes_at.take()?;
...
impl<'de> serde::Deserialize<'de> for SubscriptionsResourcePauseCollectionBehavior {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        Self::from_str(&s).map_err(|_| serde::de::Error::custom("Unknown value for SubscriptionsResourcePauseCollectionBehavior"))
    }
}
#[cfg(feature = "min-ser")]
...
impl miniserde::de::Visitor for crate::Place<SubscriptionsResourcePauseCollectionBehavior> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(SubscriptionsResourcePauseCollectionBehavior::from_str(s).map_err(|_| miniserde::Error)?);
        Ok(())
    }
}
