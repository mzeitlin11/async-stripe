...
/// Certain API requests might create multiple events.
/// For example, if you create a new subscription for a customer, you receive both a `customer.subscription.created` event and a `charge.succeeded` event.  Events occur when the state of another API resource changes.
/// The event's data field embeds the resource's state at the time of the change.
...
impl AsRef<str> for NotificationEventType {
    fn as_ref(&self) -> &str {
        self.as_str()
    }
}
...
impl std::fmt::Debug for NotificationEventType {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.write_str(self.as_str())
    }
}
impl serde::Serialize for NotificationEventType {
...
impl<'de> serde::Deserialize<'de> for NotificationEventType {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        use std::str::FromStr;
        let s: std::borrow::Cow<'de, str> = serde::Deserialize::deserialize(deserializer)?;
        Ok(Self::from_str(&s).unwrap_or(NotificationEventType::Unknown))
    }
}
