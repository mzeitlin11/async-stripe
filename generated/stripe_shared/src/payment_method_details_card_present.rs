...
const _: () = {
    ...
    impl Visitor for Place<PaymentMethodDetailsCardPresent> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: PaymentMethodDetailsCardPresentBuilder::deser_default() }))
        }
    }
    ...
    impl MapBuilder for PaymentMethodDetailsCardPresentBuilder {
        ...
        fn key(&mut self, k: &str) -> miniserde::Result<&mut dyn Visitor> {
            ...
            match k {
                ...
                "incremental_authorization_supported" => Ok(Deserialize::begin(&mut self.incremental_authorization_supported)),
                ...
                "overcapture_supported" => Ok(Deserialize::begin(&mut self.overcapture_supported)),
        ...
        fn take_out(&mut self) -> Option<Self::Out> {
            ...
            let incremental_authorization_supported = self.incremental_authorization_supported.take()?;
            let issuer = self.issuer.take()?;
            let last4 = self.last4.take()?;
            let network = self.network.take()?;
            let offline = self.offline.take()?;
            let overcapture_supported = self.overcapture_supported.take()?;
...
impl miniserde::de::Visitor for crate::Place<PaymentMethodDetailsCardPresentReadMethod> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(PaymentMethodDetailsCardPresentReadMethod::from_str(s).map_err(|_| miniserde::Error)?);
        Ok(())
    }
}
