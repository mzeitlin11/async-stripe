...
const _: () = {
    ...
    impl Visitor for Place<CheckoutSofortPaymentMethodOptions> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: CheckoutSofortPaymentMethodOptionsBuilder::deser_default() }))
        }
    }
...
impl miniserde::de::Visitor for crate::Place<CheckoutSofortPaymentMethodOptionsSetupFutureUsage> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(CheckoutSofortPaymentMethodOptionsSetupFutureUsage::from_str(s).map_err(|_| miniserde::Error)?);
        Ok(())
    }
}
