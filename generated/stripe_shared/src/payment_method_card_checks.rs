...
#[cfg(feature = "min-ser")]
pub struct PaymentMethodCardChecksBuilder {
...
const _: () = {
    ...
    impl Deserialize for PaymentMethodCardChecks {
    ...
    struct Builder<'a> {
        out: &'a mut Option<PaymentMethodCardChecks>,
        builder: PaymentMethodCardChecksBuilder,
    }

    impl Visitor for Place<PaymentMethodCardChecks> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: PaymentMethodCardChecksBuilder::deser_default() }))
        }
    }
    ...
    impl ObjectDeser for PaymentMethodCardChecks {
        type Builder = PaymentMethodCardChecksBuilder;
    }
};
