...
pub struct TaxProductRegistrationsResourceCountryOptionsEuStandardBuilder {
    place_of_supply_scheme: Option<TaxProductRegistrationsResourceCountryOptionsEuStandardPlaceOfSupplyScheme>,
}
...
const _: () = {
    ...
    impl Visitor for Place<TaxProductRegistrationsResourceCountryOptionsEuStandard> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: TaxProductRegistrationsResourceCountryOptionsEuStandardBuilder::deser_default() }))
        }
    }
    ...
    impl MapBuilder for TaxProductRegistrationsResourceCountryOptionsEuStandardBuilder {
        ...
        fn key(&mut self, k: &str) -> miniserde::Result<&mut dyn Visitor> {
            ...
            match k {
                "place_of_supply_scheme" => Ok(Deserialize::begin(&mut self.place_of_supply_scheme)),
        ...
        fn take_out(&mut self) -> Option<Self::Out> {
            let place_of_supply_scheme = self.place_of_supply_scheme.take()?;
...
impl std::str::FromStr for TaxProductRegistrationsResourceCountryOptionsEuStandardPlaceOfSupplyScheme {
    type Err = ();
...
impl std::fmt::Debug for TaxProductRegistrationsResourceCountryOptionsEuStandardPlaceOfSupplyScheme {
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        f.write_str(self.as_str())
    }
}
impl serde::Serialize for TaxProductRegistrationsResourceCountryOptionsEuStandardPlaceOfSupplyScheme {
...
impl<'de> serde::Deserialize<'de> for TaxProductRegistrationsResourceCountryOptionsEuStandardPlaceOfSupplyScheme {
...
#[cfg(feature = "min-ser")]
impl miniserde::Deserialize for TaxProductRegistrationsResourceCountryOptionsEuStandardPlaceOfSupplyScheme {
...
#[cfg(feature = "min-ser")]
impl miniserde::de::Visitor for crate::Place<TaxProductRegistrationsResourceCountryOptionsEuStandardPlaceOfSupplyScheme> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(TaxProductRegistrationsResourceCountryOptionsEuStandardPlaceOfSupplyScheme::from_str(s).map_err(|_| miniserde::Error)?);
        Ok(())
    }
}
