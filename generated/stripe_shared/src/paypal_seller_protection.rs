...
const _: () = {
    ...
    impl Visitor for Place<PaypalSellerProtection> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: PaypalSellerProtectionBuilder::deser_default() }))
        }
    }
    ...
    impl MapBuilder for PaypalSellerProtectionBuilder {
        ...
        fn take_out(&mut self) -> Option<Self::Out> {
            let dispute_categories = self.dispute_categories.take()?;
...
impl miniserde::de::Visitor for crate::Place<PaypalSellerProtectionDisputeCategories> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(PaypalSellerProtectionDisputeCategories::from_str(s).map_err(|_| miniserde::Error)?);
        Ok(())
    }
}
/// Indicates whether the transaction is eligible for PayPal's seller protection.
