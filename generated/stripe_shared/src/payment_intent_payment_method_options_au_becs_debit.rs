...
pub struct PaymentIntentPaymentMethodOptionsAuBecsDebitBuilder {
    setup_future_usage: Option<Option<PaymentIntentPaymentMethodOptionsAuBecsDebitSetupFutureUsage>>,
}
...
const _: () = {
    ...
    impl Visitor for Place<PaymentIntentPaymentMethodOptionsAuBecsDebit> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: PaymentIntentPaymentMethodOptionsAuBecsDebitBuilder::deser_default() }))
        }
    }
...
#[cfg(feature = "min-ser")]
impl miniserde::de::Visitor for crate::Place<PaymentIntentPaymentMethodOptionsAuBecsDebitSetupFutureUsage> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(PaymentIntentPaymentMethodOptionsAuBecsDebitSetupFutureUsage::from_str(s).map_err(|_| miniserde::Error)?);
        Ok(())
    }
}
