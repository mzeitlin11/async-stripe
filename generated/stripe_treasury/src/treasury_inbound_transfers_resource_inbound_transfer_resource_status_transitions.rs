...
const _: () = {
    ...
    struct Builder<'a> {
        out: &'a mut Option<TreasuryInboundTransfersResourceInboundTransferResourceStatusTransitions>,
    ...
    impl Visitor for Place<TreasuryInboundTransfersResourceInboundTransferResourceStatusTransitions> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: TreasuryInboundTransfersResourceInboundTransferResourceStatusTransitionsBuilder::deser_default() }))
        }
    }

    impl MapBuilder for TreasuryInboundTransfersResourceInboundTransferResourceStatusTransitionsBuilder {
        ...
        fn deser_default() -> Self {
            Self { canceled_at: Deserialize::default(), failed_at: Deserialize::default(), succeeded_at: Deserialize::default() }
        }
    ...
    impl ObjectDeser for TreasuryInboundTransfersResourceInboundTransferResourceStatusTransitions {
        type Builder = TreasuryInboundTransfersResourceInboundTransferResourceStatusTransitionsBuilder;
    }
};
