...
const _: () = {
    ...
    impl Visitor for Place<PaymentFlowsPrivatePaymentMethodsAlipay> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: PaymentFlowsPrivatePaymentMethodsAlipayBuilder::deser_default() }))
        }
    }

    impl MapBuilder for PaymentFlowsPrivatePaymentMethodsAlipayBuilder {
        type Out = PaymentFlowsPrivatePaymentMethodsAlipay;
    ...
    impl ObjectDeser for PaymentFlowsPrivatePaymentMethodsAlipay {
        type Builder = PaymentFlowsPrivatePaymentMethodsAlipayBuilder;
    }
};
