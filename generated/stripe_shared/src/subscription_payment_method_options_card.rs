...
const _: () = {
    ...
    impl Visitor for Place<SubscriptionPaymentMethodOptionsCard> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: SubscriptionPaymentMethodOptionsCardBuilder::deser_default() }))
        }
    }
...
impl miniserde::de::Visitor for crate::Place<SubscriptionPaymentMethodOptionsCardNetwork> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(SubscriptionPaymentMethodOptionsCardNetwork::from_str(s).map_err(|_| miniserde::Error)?);
        Ok(())
    }
}
/// We strongly recommend that you rely on our SCA Engine to automatically prompt your customers for authentication based on risk level and [other requirements](https://stripe.com/docs/strong-customer-authentication).
...
#[cfg(feature = "min-ser")]
impl miniserde::de::Visitor for crate::Place<SubscriptionPaymentMethodOptionsCardRequestThreeDSecure> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(SubscriptionPaymentMethodOptionsCardRequestThreeDSecure::from_str(s).map_err(|_| miniserde::Error)?);
        Ok(())
    }
}
