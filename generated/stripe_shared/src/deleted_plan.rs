...
const _: () = {
    ...
    struct Builder<'a> {
        out: &'a mut Option<DeletedPlan>,
        builder: DeletedPlanBuilder,
    }

    impl Visitor for Place<DeletedPlan> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: DeletedPlanBuilder::deser_default() }))
        }
    }

    impl MapBuilder for DeletedPlanBuilder {
        type Out = DeletedPlan;
