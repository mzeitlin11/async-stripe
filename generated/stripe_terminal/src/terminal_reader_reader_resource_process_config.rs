...
const _: () = {
    ...
    impl Visitor for Place<TerminalReaderReaderResourceProcessConfig> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: TerminalReaderReaderResourceProcessConfigBuilder::deser_default() }))
        }
    }
    ...
    impl MapBuilder for TerminalReaderReaderResourceProcessConfigBuilder {
        ...
        fn take_out(&mut self) -> Option<Self::Out> {
            let skip_tipping = self.skip_tipping.take()?;
            let tipping = self.tipping.take()?;

            Some(Self::Out { skip_tipping, tipping })
        }
    }
