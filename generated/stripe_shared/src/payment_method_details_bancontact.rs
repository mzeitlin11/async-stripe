...
const _: () = {
    ...
    impl Visitor for Place<PaymentMethodDetailsBancontact> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: PaymentMethodDetailsBancontactBuilder::deser_default() }))
        }
    }
...
impl<'de> serde::Deserialize<'de> for PaymentMethodDetailsBancontactPreferredLanguage {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        Self::from_str(&s).map_err(|_| serde::de::Error::custom("Unknown value for PaymentMethodDetailsBancontactPreferredLanguage"))
    }
}
#[cfg(feature = "min-ser")]
...
impl miniserde::de::Visitor for crate::Place<PaymentMethodDetailsBancontactPreferredLanguage> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(PaymentMethodDetailsBancontactPreferredLanguage::from_str(s).map_err(|_| miniserde::Error)?);
        Ok(())
    }
}
