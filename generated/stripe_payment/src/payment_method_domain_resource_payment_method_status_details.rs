...
const _: () = {
    ...
    impl Visitor for Place<PaymentMethodDomainResourcePaymentMethodStatusDetails> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: PaymentMethodDomainResourcePaymentMethodStatusDetailsBuilder::deser_default() }))
        }
    }
    ...
    impl MapBuilder for PaymentMethodDomainResourcePaymentMethodStatusDetailsBuilder {
        ...
        fn key(&mut self, k: &str) -> miniserde::Result<&mut dyn Visitor> {
            ...
            match k {
                "error_message" => Ok(Deserialize::begin(&mut self.error_message)),
