...
const _: () = {
    ...
    impl Visitor for Place<TaxProductResourceCustomerDetailsResourceTaxId> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: TaxProductResourceCustomerDetailsResourceTaxIdBuilder::deser_default() }))
        }
    }
...
impl miniserde::de::Visitor for crate::Place<TaxProductResourceCustomerDetailsResourceTaxIdType> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(TaxProductResourceCustomerDetailsResourceTaxIdType::from_str(s).map_err(|_| miniserde::Error)?);
        Ok(())
    }
}
