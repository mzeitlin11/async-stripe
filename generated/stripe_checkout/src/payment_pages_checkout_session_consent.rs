...
const _: () = {
    ...
    impl Visitor for Place<PaymentPagesCheckoutSessionConsent> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: PaymentPagesCheckoutSessionConsentBuilder::deser_default() }))
        }
    }
...
impl miniserde::de::Visitor for crate::Place<PaymentPagesCheckoutSessionConsentPromotions> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(PaymentPagesCheckoutSessionConsentPromotions::from_str(s).map_err(|_| miniserde::Error)?);
        Ok(())
    }
}
/// If `accepted`, the customer in this Checkout Session has agreed to the merchant's terms of service.
...
impl miniserde::de::Visitor for crate::Place<PaymentPagesCheckoutSessionConsentTermsOfService> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(PaymentPagesCheckoutSessionConsentTermsOfService::from_str(s).map_err(|_| miniserde::Error)?);
        Ok(())
    }
}
