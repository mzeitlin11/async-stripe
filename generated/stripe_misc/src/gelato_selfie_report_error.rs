...
const _: () = {
    ...
    impl Visitor for Place<GelatoSelfieReportError> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: GelatoSelfieReportErrorBuilder::deser_default() }))
        }
    }
...
impl<'de> serde::Deserialize<'de> for GelatoSelfieReportErrorCode {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        Self::from_str(&s).map_err(|_| serde::de::Error::custom("Unknown value for GelatoSelfieReportErrorCode"))
    }
}
#[cfg(feature = "min-ser")]
