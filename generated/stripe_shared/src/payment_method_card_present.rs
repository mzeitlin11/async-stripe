...
const _: () = {
    ...
    impl Visitor for Place<PaymentMethodCardPresent> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: PaymentMethodCardPresentBuilder::deser_default() }))
        }
    }
    ...
    impl MapBuilder for PaymentMethodCardPresentBuilder {
        ...
        fn take_out(&mut self) -> Option<Self::Out> {
            ...
            Some(Self::Out { brand, cardholder_name, country, description, exp_month, exp_year, fingerprint, funding, iin, issuer, last4, networks, read_method })
        }
    }
...
impl AsRef<str> for PaymentMethodCardPresentReadMethod {
    fn as_ref(&self) -> &str {
        self.as_str()
    }
}
...
impl<'de> serde::Deserialize<'de> for PaymentMethodCardPresentReadMethod {
    fn deserialize<D: serde::Deserializer<'de>>(deserializer: D) -> Result<Self, D::Error> {
        ...
        Self::from_str(&s).map_err(|_| serde::de::Error::custom("Unknown value for PaymentMethodCardPresentReadMethod"))
    }
}
#[cfg(feature = "min-ser")]
...
impl miniserde::de::Visitor for crate::Place<PaymentMethodCardPresentReadMethod> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(PaymentMethodCardPresentReadMethod::from_str(s).map_err(|_| miniserde::Error)?);
        Ok(())
    }
}
