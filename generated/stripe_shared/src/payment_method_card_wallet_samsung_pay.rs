...
const _: () = {
    ...
    impl Visitor for Place<PaymentMethodCardWalletSamsungPay> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: PaymentMethodCardWalletSamsungPayBuilder::deser_default() }))
        }
    }

    impl MapBuilder for PaymentMethodCardWalletSamsungPayBuilder {
        type Out = PaymentMethodCardWalletSamsungPay;
    ...
    impl ObjectDeser for PaymentMethodCardWalletSamsungPay {
