...
const _: () = {
    ...
    impl Visitor for Place<SubscriptionPendingInvoiceItemInterval> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: SubscriptionPendingInvoiceItemIntervalBuilder::deser_default() }))
        }
    }
...
impl miniserde::de::Visitor for crate::Place<SubscriptionPendingInvoiceItemIntervalInterval> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(SubscriptionPendingInvoiceItemIntervalInterval::from_str(s).map_err(|_| miniserde::Error)?);
        Ok(())
    }
}
