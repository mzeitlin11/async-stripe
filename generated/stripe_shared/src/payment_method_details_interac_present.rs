...
const _: () = {
    ...
    impl Visitor for Place<PaymentMethodDetailsInteracPresent> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: PaymentMethodDetailsInteracPresentBuilder::deser_default() }))
        }
    }
...
impl miniserde::de::Visitor for crate::Place<PaymentMethodDetailsInteracPresentReadMethod> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(PaymentMethodDetailsInteracPresentReadMethod::from_str(s).map_err(|_| miniserde::Error)?);
        Ok(())
    }
}
