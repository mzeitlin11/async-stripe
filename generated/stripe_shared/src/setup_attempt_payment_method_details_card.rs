...
const _: () = {
    ...
    impl Visitor for Place<SetupAttemptPaymentMethodDetailsCard> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: SetupAttemptPaymentMethodDetailsCardBuilder::deser_default() }))
        }
    }
    ...
    impl MapBuilder for SetupAttemptPaymentMethodDetailsCardBuilder {
        ...
        fn take_out(&mut self) -> Option<Self::Out> {
            ...
            Some(Self::Out { brand, checks, country, description, exp_month, exp_year, fingerprint, funding, iin, issuer, last4, network, three_d_secure, wallet })
        }
    }
