...
const _: () = {
    ...
    impl MapBuilder for PaymentIntentNextActionBuilder {
        ...
        fn key(&mut self, k: &str) -> miniserde::Result<&mut dyn Visitor> {
            ...
            match k {
                "alipay_handle_redirect" => Ok(Deserialize::begin(&mut self.alipay_handle_redirect)),
                "boleto_display_details" => Ok(Deserialize::begin(&mut self.boleto_display_details)),
                "card_await_notification" => Ok(Deserialize::begin(&mut self.card_await_notification)),
                "cashapp_handle_redirect_or_display_qr_code" => Ok(Deserialize::begin(&mut self.cashapp_handle_redirect_or_display_qr_code)),
                "display_bank_transfer_instructions" => Ok(Deserialize::begin(&mut self.display_bank_transfer_instructions)),
                "konbini_display_details" => Ok(Deserialize::begin(&mut self.konbini_display_details)),
                "oxxo_display_details" => Ok(Deserialize::begin(&mut self.oxxo_display_details)),
                "paynow_display_qr_code" => Ok(Deserialize::begin(&mut self.paynow_display_qr_code)),
                "pix_display_qr_code" => Ok(Deserialize::begin(&mut self.pix_display_qr_code)),
                "promptpay_display_qr_code" => Ok(Deserialize::begin(&mut self.promptpay_display_qr_code)),
                ...
                "wechat_pay_display_qr_code" => Ok(Deserialize::begin(&mut self.wechat_pay_display_qr_code)),
                "wechat_pay_redirect_to_android_app" => Ok(Deserialize::begin(&mut self.wechat_pay_redirect_to_android_app)),
                "wechat_pay_redirect_to_ios_app" => Ok(Deserialize::begin(&mut self.wechat_pay_redirect_to_ios_app)),
