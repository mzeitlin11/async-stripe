...
const _: () = {
    ...
    impl Visitor for Place<TreasuryReceivedDebitsResourceDebitReversalLinkedFlows> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: TreasuryReceivedDebitsResourceDebitReversalLinkedFlowsBuilder::deser_default() }))
        }
    }
    ...
    impl MapBuilder for TreasuryReceivedDebitsResourceDebitReversalLinkedFlowsBuilder {
        ...
        fn key(&mut self, k: &str) -> miniserde::Result<&mut dyn Visitor> {
            ...
            match k {
                "issuing_dispute" => Ok(Deserialize::begin(&mut self.issuing_dispute)),
        ...
        fn take_out(&mut self) -> Option<Self::Out> {
            let issuing_dispute = self.issuing_dispute.take()?;
