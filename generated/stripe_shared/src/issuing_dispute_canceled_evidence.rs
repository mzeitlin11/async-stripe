...
const _: () = {
    ...
    impl Visitor for Place<IssuingDisputeCanceledEvidence> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: IssuingDisputeCanceledEvidenceBuilder::deser_default() }))
        }
    }
    ...
    impl MapBuilder for IssuingDisputeCanceledEvidenceBuilder {
        ...
        fn key(&mut self, k: &str) -> miniserde::Result<&mut dyn Visitor> {
            ...
            match k {
                ...
                "cancellation_policy_provided" => Ok(Deserialize::begin(&mut self.cancellation_policy_provided)),
        ...
        fn take_out(&mut self) -> Option<Self::Out> {
            ...
            let cancellation_policy_provided = self.cancellation_policy_provided.take()?;
...
impl miniserde::de::Visitor for crate::Place<IssuingDisputeCanceledEvidenceProductType> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(IssuingDisputeCanceledEvidenceProductType::from_str(s).map_err(|_| miniserde::Error)?);
        Ok(())
    }
}
/// Result of cardholder's attempt to return the product.
...
impl miniserde::de::Visitor for crate::Place<IssuingDisputeCanceledEvidenceReturnStatus> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(IssuingDisputeCanceledEvidenceReturnStatus::from_str(s).map_err(|_| miniserde::Error)?);
        Ok(())
    }
}
