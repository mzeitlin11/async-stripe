...
pub fn gen_inferred_params(
    ...
    for (_, params) in state.inferred_parameters.clone() {
        ...
        writeln!(out, "pub struct {}<'a> {{", params.rust_type).unwrap();
            ...
            match param_name {
                ...
                _ => {
                        ...
                        meta.field_to_rust_type(params_schema.as_str(), param_name)
                    {
                        ...
                        write_out_field(out, param_rename, rust_type, true);
                        ...
                        || param["schema"]["anyOf"].is_array()
                    {
                        ...
                        writeln!(out, "pub {param_rename}: {rust_type},").unwrap();
                    } else if required {
                        panic!(
    ...
    while !generated_objects.is_empty() {
        ...
        if let Some(array) = schema["anyOf"].as_array() {
            ...
            for value in array {
                ...
                writeln!(out, "pub {variant_name}({type_name}),").unwrap();
            }
            out.push_str("}\n");
        }
...
pub fn gen_field(
    ...
    write_out_field(&mut out, &field_rename, &rust_type, true);
    out
}
