...
const _: () = {
    ...
    impl Visitor for Place<PortalSubscriptionUpdate> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: PortalSubscriptionUpdateBuilder::deser_default() }))
        }
    }
    ...
    impl MapBuilder for PortalSubscriptionUpdateBuilder {
        ...
        fn deser_default() -> Self {
            Self { default_allowed_updates: Deserialize::default(), enabled: Deserialize::default(), products: Deserialize::default(), proration_behavior: Deserialize::default() }
        }

        fn take_out(&mut self) -> Option<Self::Out> {
            let default_allowed_updates = self.default_allowed_updates.take()?;
...
impl miniserde::de::Visitor for crate::Place<PortalSubscriptionUpdateDefaultAllowedUpdates> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(PortalSubscriptionUpdateDefaultAllowedUpdates::from_str(s).map_err(|_| miniserde::Error)?);
        Ok(())
    }
}
/// Determines how to handle prorations resulting from subscription updates.
...
impl miniserde::de::Visitor for crate::Place<PortalSubscriptionUpdateProrationBehavior> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(PortalSubscriptionUpdateProrationBehavior::from_str(s).map_err(|_| miniserde::Error)?);
        Ok(())
    }
}
