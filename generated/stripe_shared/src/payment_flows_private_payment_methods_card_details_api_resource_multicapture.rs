...
const _: () = {
    ...
    impl Visitor for Place<PaymentFlowsPrivatePaymentMethodsCardDetailsApiResourceMulticapture> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: PaymentFlowsPrivatePaymentMethodsCardDetailsApiResourceMulticaptureBuilder::deser_default() }))
        }
    }
...
impl std::str::FromStr for PaymentFlowsPrivatePaymentMethodsCardDetailsApiResourceMulticaptureStatus {
    type Err = ();
...
impl serde::Serialize for PaymentFlowsPrivatePaymentMethodsCardDetailsApiResourceMulticaptureStatus {
...
impl<'de> serde::Deserialize<'de> for PaymentFlowsPrivatePaymentMethodsCardDetailsApiResourceMulticaptureStatus {
...
#[cfg(feature = "min-ser")]
impl miniserde::Deserialize for PaymentFlowsPrivatePaymentMethodsCardDetailsApiResourceMulticaptureStatus {
...
#[cfg(feature = "min-ser")]
impl miniserde::de::Visitor for crate::Place<PaymentFlowsPrivatePaymentMethodsCardDetailsApiResourceMulticaptureStatus> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(PaymentFlowsPrivatePaymentMethodsCardDetailsApiResourceMulticaptureStatus::from_str(s).map_err(|_| miniserde::Error)?);
        Ok(())
    }
}
