...
const _: () = {
    ...
    impl Visitor for Place<SetupAttemptPaymentMethodDetailsIdeal> {
        fn map(&mut self) -> Result<Box<dyn Map + '_>> {
            Ok(Box::new(Builder { out: &mut self.out, builder: SetupAttemptPaymentMethodDetailsIdealBuilder::deser_default() }))
        }
    }
...
impl miniserde::de::Visitor for crate::Place<SetupAttemptPaymentMethodDetailsIdealBank> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(SetupAttemptPaymentMethodDetailsIdealBank::from_str(s).unwrap_or(SetupAttemptPaymentMethodDetailsIdealBank::Unknown));
        Ok(())
    }
}
/// The Bank Identifier Code of the customer's bank.
...
impl miniserde::de::Visitor for crate::Place<SetupAttemptPaymentMethodDetailsIdealBic> {
    fn string(&mut self, s: &str) -> miniserde::Result<()> {
        use std::str::FromStr;
        self.out = Some(SetupAttemptPaymentMethodDetailsIdealBic::from_str(s).unwrap_or(SetupAttemptPaymentMethodDetailsIdealBic::Unknown));
        Ok(())
    }
}
